    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                       <form class="needs-validation" method="POST" action="{{ route('backend.product-image.update',parameters: $product_image->id) }}"
                                enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <input type="hidden" name="product_id" value="{{$product_image->product_id}}">
                               <div class="row mb-3">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">Şəkil (24x24)</label>
                                    <div class="col-sm-10">
                                        <div id="image-preview" class="image-preview">
                                            <label for="image-upload" id="image-label">Faylı seç</label>
                                            <input type="file" name="image" id="image-upload" />
                                            <img style="width:200px" src="{{$product_image?->image}}" alt="">
                                        </div>
                                    </div>
                                </div>
