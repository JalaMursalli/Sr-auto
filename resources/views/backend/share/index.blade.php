                    ...
                    <div class="card">
                        <div class="card-header align-items-end d-flex justify-content-between">
                           <div>
                            ...
                            <div class="card-header-form">
                                <form action="{{ route('backend.share.index') }}" method="GET" class="d-flex"
                            ...
                            <a href="{{ route('backend.share.create') }}" class="btn btn-primary">Yeni Sosial Şəbəkə yarat</a>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @forelse ($shares as $key => $share)
                                        <tr>
                                            <td>{{ $shares->firstItem() + $key }}</td>
                                            <td>
                                                <img src="{{ asset($share->icon) }}" alt=""
                                            ...
                                            <td class="text-end">
                                                <div class="d-flex justify-content-end " style="gap: 20px">
                                                    <a href="{{ route('backend.share.edit', $share?->id ?? '') }}"
                                                        class="btn btn-warning btn-sm">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </a>
                                                    <form
                                                        action="{{ route('backend.share.destroy', $share?->id ?? '') }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <div class="d-flex salam justify-content-end">
                                {{ $shares->appends(['search' => request('search')])->links() }}
                            </div>
                        </div>
                    </div>
                </div>
